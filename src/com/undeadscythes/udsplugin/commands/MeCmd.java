...
public class MeCmd implements CommandExecutor {
    ...
    public MeCmd(final UDSPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public final boolean onCommand(final CommandSender commandSender, final Command command, final String label, final String[] args) {
        ...
        if(sender.hasPermission("udsplugin.me")) {
            if(args.length > 0) {
                final UDSString message = new UDSString(StringUtils.join(args, " "));
                if(!message.censor()) {
                    server.broadcastMessage(Color.TEXT + "* " + UDSPlugin.getPlayers().get(senderName).getNick() + " " + message.getString());
                } else {
